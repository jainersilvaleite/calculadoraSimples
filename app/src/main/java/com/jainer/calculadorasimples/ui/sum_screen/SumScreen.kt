...
@Composable
fun CompactSumScreen(
    navController: NavController,
    modifier: Modifier = Modifier
) {
    val viewModel: SumScreenViewModel = viewModel()
    ...
    Column(
        ...
        HeaderText(text = stringResource(R.string.homeOption_sum))
        ...
        CalculateButton(
            onClick = {
                uiState.changeResult(
                    uiState.sum(firstNumber, secondNumber)
                )
            }
        )
        Spacer(modifier = Modifier.height(50.dp))
...
fun SumScreenPreview() {
    CalculadoraSimplesTheme {
        SumScreen(rememberNavController())
    }
}