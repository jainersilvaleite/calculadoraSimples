...
fun CompactMultiScreen(
    ...
    val viewModel: MultiScreenViewModel = viewModel()
    ...
    Column(
        ...
        HeaderText(text = stringResource(R.string.homeOption_multi))
        ...
        NumbersBox(
            ...
            operationIcon = R.drawable.operacao_multiplicar
        )
        Spacer(modifier = Modifier.height(20.dp))
        CalculateButton(
            onClick = {
                uiState.changeResult(
                    uiState.multi(firstNumber, secondNumber)
                )
            }
        )
        Spacer(modifier = Modifier.height(50.dp))
...
@Composable
fun MediumToExpandedMultiScreen(
    navController: NavController,
    modifier: Modifier = Modifier
) {
    val viewModel: MultiScreenViewModel = viewModel()
...
fun MultiScreenPreview() {
    CalculadoraSimplesTheme {
        MultiScreen(rememberNavController())
    }
}